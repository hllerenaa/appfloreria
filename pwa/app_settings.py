...
PWA_APP_NAME = getattr(settings, 'PWA_APP_NAME', f'{confi_.nombre_empresa}')
PWA_APP_DESCRIPTION = getattr(settings, 'PWA_APP_DESCRIPTION', f'{confi_.descripcion}')
PWA_APP_ROOT_URL = resolve_url(getattr(settings, 'PWA_APP_ROOT_URL', _PWA_SCRIPT_PREFIX))
PWA_APP_THEME_COLOR = getattr(settings, 'PWA_APP_THEME_COLOR', '#F1948A')
...
PWA_APP_ICONS = getattr(settings, 'PWA_APP_ICONS', [
    {
        'src': '/static/images/icons/icon-72x72.png' if not confi_.ico else confi_.ico.url,
        'size': '72x72'
    },
    {
        'src': '/static/images/icons/icon-96x96.png' if not confi_.ico else confi_.ico.url,
        'size': '96x96'
    },
    {
        'src': '/static/images/icons/icon-128x128.png' if not confi_.ico else confi_.ico.url,
        'size': '128x128'
    },
    {
        'src': '/static/images/icons/icon-144x144.png' if not confi_.ico else confi_.ico.url,
        'size': '144x144'
    },
    {
        'src': '/static/images/icons/icon-152x152.png' if not confi_.ico else confi_.ico.url,
        'size': '152x152'
    },
    {
        'src': '/static/images/icons/icon-192x192.png' if not confi_.ico else confi_.ico.url,
        'size': '192x192'
    },
    {
        'src': '/static/images/icons/icon-384x384.png' if not confi_.ico else confi_.ico.url,
        'size': '384x384'
    },
    {
        'src': '/static/images/icons/icon-512x512.png' if not confi_.ico else confi_.ico.url,
        'size': '512x512'
    }
])
