...
from .models import CuentaFinancieraEmpresa
from django.contrib import messages
from datetime import date, datetime

@login_required
@secure_module
def cuentaFinancieraView(request):
    data = {'titulo': 'Cuenta Financiera',
            'modulo': 'Financiero',
            'ruta': request.path,

            }
    model = CuentaFinancieraEmpresa
    Formulario = CuentaFinancieraForm
    nombre_para_audit = 'num_cuenta'
    ...
    if request.method == 'POST':
        ...
        if 'action' in request.GET:
            ...
            if action == 'add':
                if can_add:
                    ...
                    return render(request, 'financiero/cuenta_financiera/form.html', data)
                else:
        ...
        if can_view:
            ...
            if criterio:
                filtros = filtros & Q(num_cuenta__icontains=criterio)
                data["criterio"] = criterio
                url_vars += '&criterio=' + criterio
            listado = model.objects.filter(filtros)
            data["url_vars"] = url_vars
            mi_paginador(request, listado, 40, data)
            return render(request, 'financiero/cuenta_financiera/listado.html', data)
        else:
            messages.error(request, "No tienes permisos para ver {}".format(data["titulo"]))
            return redirect('/')