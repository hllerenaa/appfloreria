            ...
            with transaction.atomic():
                if action == 'add':
                    ...
                    if can_change:
                        ...
                        res_json.append({"error": True, "message": "No tienes permisos para esta acción"})
                elif action == 'crearperfilestudiante':
                    if can_change:
                        user = model.objects.get(pk=int(request.POST['id']))
                        if not user.es_cliente():
                            perfil_ = PerfilCliente(usuario=user)
                            perfil_.save()
                            salva_auditoria(request, user, action, f'Creo perfil estudiante {user.get_full_name()}')
                            messages.success(request, "Perfil estudiante habilitado.")
                        else:
                            messages.error(request, "Usuario ya cuenta con perfil estudiante")
                    else:
                    ...
                    if can_change:
                        ...
                        user.save()
                        salva_auditoria(request, user, action, user.get_full_name())
